...
export function RabiscoEditor({ initialProject }: { initialProject: ProjectDTO }) {
  const router = useRouter();
  const [project, setProject] = useState(initialProject);
  const [thoughtDraft, setThoughtDraft] = useState(initialProject.rabiscoThought ?? '');
  ...
  const refresh = useCallback(async () => {
    ...
    setThoughtDraft(data.project.rabiscoThought ?? '');
    return data.project;
  }, [project.id]);
  ...
  const script =
    project.rabiscoScriptJson && typeof project.rabiscoScriptJson === 'object'
      ? (project.rabiscoScriptJson as RabiscoScript)
      : null;
  ...
  return (
    <main>
      ...
      <p className="muted" style={{ marginTop: 0, marginBottom: 24 }}>
        {KIND_LABEL[project.kind] ?? project.kind} · totalmente automático: só clique em Produzir
      </p>
          ...
          <div className="row" style={{ alignItems: 'flex-start' }}>
            ...
            <div>
              <p className="muted" style={{ marginTop: 0 }}>
                {project.outputSizeByte ? `${(project.outputSizeByte / 1024 / 1024).toFixed(1)} MB` : ''} ·
      ...
      <div className="card">
        ...
        <div style={{ marginTop: 16 }}>
          <label htmlFor="ra-watermark">Marca d&apos;água</label>
          <input
            id="ra-watermark"
            type="text"
      ...
      {script?.hook || script?.narration ? (
        <div className="card">
          ...
          {script.narration ? <p className="muted" style={{ whiteSpace: 'pre-line' }}>{script.narration}</p> : null}
        </div>
      ) : null}
