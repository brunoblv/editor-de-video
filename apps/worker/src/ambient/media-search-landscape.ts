...
import type { StockHit } from '../v2/media-search.js';

function scoreLandscape(opts: {
  width: number;
  height: number;
  durationSec: number;
}): number {
  ...
  if (width >= height) score += 25;
  else if (height > 0) score += 5;
  if (width >= 1920 || height >= 1080) score += 15;
  else if (width >= 1280 || height >= 720) score += 8;
  if (durationSec >= 8 && durationSec <= 60) score += 10;
  else if (durationSec > 60) score += 6;
  return Math.min(100, score);
}

async function searchPexelsLandscape(query: string): Promise<StockHit[]> {
  ...
  for (const video of data.videos ?? []) {
    ...
    hits.push({
      ...
      height,
      durationSec,
      score: scoreLandscape({ width, height, durationSec }),
    });
  }
  return hits;
}

async function searchPixabayLandscape(query: string): Promise<StockHit[]> {
...
/** Busca stock em landscape (16:9) para ambient. */
export async function searchBestStockLandscape(
  query: string,
  usedUrls: Set<string>,
): Promise<StockHit> {
  if (!config.media.pexelsApiKey && !config.media.pixabayApiKey) {
    throw new Error(
      'Configure PEXELS_API_KEY e/ou PIXABAY_API_KEY no .env para buscar mídia visual.',
    );
  }

  const [pexels, pixabay] = await Promise.all([
    searchPexelsLandscape(query),
    searchPixabayLandscape(query),
  ]);
  ...
  const best = ranked[0];
  if (!best) {
    throw new Error(`Nenhuma mídia landscape encontrada para "${query}".`);
  }
  return best;
}
